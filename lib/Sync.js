...
Sync.prototype.setupRun = function(remote) {
    ask({
        client_id: {
            question: 'Client ID: ',
            required: true
        ...
        scopes: {
            question: 'Script scopes (empty line ends scope input): ',
            required: true
        }
    }, function(answers) {
        console.log(answers);
...
Sync.prototype.run = function(remote, fn, ...args) {
    // override Auth.CREDENTIALS and extend Auth.tokens with locals
    // execute function
};

function ask(questions, callback) {
    var keys = _.keys(questions);
    var answers = {};
    var rl = readline.createInterface({
        input: process.stdin,
        output: process.stdout
    });
    ask(keys.shift(), function handleAnswer(key, answer) {
        answers[key] = answer;
        if(keys.length)
            ask(keys.shift(), handleAnswer);
        else
            callback(answers);
    });
    function ask(collection, key, callback) {
        ...
        rl.question(question.query, (answer) => {
            if(question.array) {
                if(!_.empty(answer.trim())) {
                    collection.push(answer);
                } else if(question.required && collection.length) {
                    return callback(key, collection);
                }
                return ask(collection, key, callback);
            }
            return callback(key, answer);
        });
    }
}
